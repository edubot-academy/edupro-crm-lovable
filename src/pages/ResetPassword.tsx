...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) {
      toast({ title: 'Токен табылган жок', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Сырсөз ийгиликтүү өзгөртүлдү!' });
      navigate('/login');
    } catch (err: unknown) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            <p className="text-destructive">Жараксыз сырсөз калыбына келтирүү шилтемеси</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-elevated border-border/50">
        <CardHeader className="text-center space-y-2 pb-2">
          ...
          <CardDescription>Жаңы сырсөзүңүздү киргизиңиз</CardDescription>
        </CardHeader>
        <CardContent>
